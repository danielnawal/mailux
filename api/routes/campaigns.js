...
import { sendCampaignEmailToContact } from '../services/campaign-send.js';
...
// Normaliza el límite diario del goteo: número entero > 0, o null (envío normal).
function parseDailyLimit(v) {
...
router.post('/', (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO campaigns (distributor_id, list_id, name, subject, from_name, from_email, body_html, segment_key, segment_value, daily_limit)
    VALUES (?,?,?,?,?,?,?,?,?,?)
  `).run(req.user.distributor_id, list_id || null, name, subject, from_name, from_email, body_html, segKey, segVal, dailyLimit);
...
router.put('/:id', (req, res) => {
  ...
  const dailyLimit = parseDailyLimit(req.body.daily_limit);
  db.prepare(`
    UPDATE campaigns SET name=?, subject=?, from_name=?, from_email=?, body_html=?, list_id=?, segment_key=?, segment_value=?, daily_limit=?
    WHERE id=?
  `).run(name, subject, from_name, from_email, body_html, list_id || null, segKey, segVal, dailyLimit, campaign.id);
...
async function executeCampaignSend(campaignId) {
  ...
  for (const contact of contacts) {
    try {
      const html = await sendCampaignEmailToContact(campaign, contact);
...
router.post('/:id/send', async (req, res) => {
  ...
  if (campaign.daily_limit) return res.status(400).json({ error: 'Esta campaña es de envío por goteo. Actívala con el botón "Activar goteo", no con "Enviar ahora".' });
...
// Activar el envío por GOTEO: pone la campaña 'sending'. El cron de goteo (services/drip.js)
// enviará hasta daily_limit correos hoy y el resto en los días siguientes. Reutiliza los
// botones Pausar/Reanudar existentes (sending<->paused). Solo aplica a campañas con daily_limit.
router.post('/:id/drip/start', async (req, res) => {
  const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ? AND distributor_id = ?').get(req.params.id, req.user.distributor_id);
  if (!campaign) return res.status(404).json({ error: 'Campaña no encontrada' });
  if (!campaign.daily_limit) return res.status(400).json({ error: 'Esta campaña no tiene envío por goteo configurado (define un máximo por día).' });
  if (!campaign.list_id) return res.status(400).json({ error: 'Selecciona una lista antes de activar el goteo' });
  if (campaign.status === 'sent') return res.status(400).json({ error: 'Esta campaña ya terminó de enviarse' });
  if (campaign.status === 'sending') return res.status(400).json({ error: 'El goteo ya está activo' });

  const contacts = eligibleContacts(campaign, false);
  if (!contacts.length) return res.status(400).json({ error: 'La lista no tiene contactos válidos para enviar' });

  // last_send_date = NULL para que el primer lote salga hoy mismo.
  db.prepare("UPDATE campaigns SET status='sending', last_send_date=NULL, total=? WHERE id=?").run(contacts.length, campaign.id);
  res.json({ ok: true, total: contacts.length, daily_limit: campaign.daily_limit, message: `Goteo activado: ${campaign.daily_limit} correos por día.` });

  // Dispara el primer lote de inmediato (sin esperar al tick del cron).
  const { processDripCampaigns } = await import('../services/drip.js');
